...
/// # use hacksilver::*;
/// # use vector::*;
/// let bb = Bounds2D::new(ivec3(1, 2, 3), ivec3(4, 5, 6));
/// assert_eq!(bb.vertices().collect::<Vec<_>>, vec![]);
///
/// ```
impl Bounds2D<i32> {
	pub fn vertices(&self) -> impl Iterator<Item = ivec3> {
		let size = self.size();
		let min = self.min;
		cross3(0..=1, 0..=1, 0..=1).map(move |(x, y, z)| min + Vector3::new(x, y, z) * size)
	}
}
	...
	T: Copy,
{
	...
	/// # use vector::*;
	/// let bb = Bounds2D::new(vec3(1.0, 2.0, 3.0), vec3(4.0, 5.0, 6.0));
	/// assert_eq!(bb.convert::<i32>(), Bounds2D::new(ivec3(1, 2, 3), ivec3(4, 5, 6)));
	///
	/// ```
	pub fn convert<U>(&self) -> Bounds2D<U>
	where
		T: Convert<U>,
		U: Copy + PartialOrd,
	{
		Bounds2D::new(self.min.convert(), self.max.convert())
	}
}
	...
	T: Number,
{
	...
	/// # use hacksilver::raytrace::*;
	/// # use vector::*;
	/// let a = Bounds2D::new(ivec3(1, 2, 3), ivec3(4, 5, 6));
	/// let b = a.add(ivec3(-1, 20, 4));
	/// assert_eq!(b, Bounds2D::new(ivec3(-1, 2, 3), ivec3(4, 20, 6)));
	/// ```
	#[must_use]
	...
	/// # use hacksilver::raytrace::*;
	/// # use vector::*;
	/// let a = Bounds2D::new(vec3(1.0, 2.0, 3.0), vec3(4.0, 5.0, 6.0));
	/// let b = Bounds2D::new(vec3(-1.0, 5.0, 4.0), vec3(40.0, 20.0, 6.0));
	/// assert_eq!(a.join(&b), Bounds2D::new(vec3(-1.0, 2.0, 3.0), vec3(40.0, 20.0, 6.0)));
	/// ```
	#[must_use]
	...
	/// let bb = Bounds2D::from(points.iter());
	/// assert_eq!(bb, Some(Bounds2D::new(vec3(-1.0, 2.0, 3.0), vec3(4.0, 20.0, 6.0))));
	///
...
pub type Bounds2D = Bounds2D<f64>;
pub type Bounds2D = Bounds2D<f32>;
...
mod test {
	...
	fn degenerate() {
		...
		let bb = Bounds2D::new(dvec3(-1., -1., 0.), dvec3(1., 1., 0.));
